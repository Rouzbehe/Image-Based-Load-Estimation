...
files=dir(fullfile('SegmentedImages', '*.png'));
...
PCA_Scaled_segmented_cropped_line=cell(length(files),1);


for jj=1:length(files)
    
%     filename=files(jj).name;
    fname = strcat('./SegmentedImages/',char(FILES(jj)));
...
%         Island_Scaled_segmented_ex_postcap_cropped_line{jj,1}=I;
        PCA_Scaled_segmented_cropped_line{jj,1}=I;

clear n m I
end
save PCA_Scaled_segmented_cropped_line.mat;
toc