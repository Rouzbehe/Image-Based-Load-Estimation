...
files=dir(fullfile('SegmentedImages_ex_postcap', '*.png'));
...
PCA_Scaled_segmented_cropped_line_ex_postcap=cell(length(files),1);


for jj=1:length(files)
    
%     filename=files(jj).name;
    fname = strcat('./SegmentedImages_ex_postcap/',char(FILES(jj)));
...
%     BW2=logical(BW2);
    ...
    Widden_fname = strcat('./Widden_Sorted_Segmented_BeamCropped_line_ex_postcap/',char(FILES(jj)));
...
%         Island_Scaled_segmented_ex_postcap_cropped_line{jj,1}=I;
        PCA_Scaled_segmented_cropped_line_ex_postcap{jj,1}=I;

clear n m I
end
save PCA_Scaled_segmented_cropped_line_ex_postcap.mat;
toc