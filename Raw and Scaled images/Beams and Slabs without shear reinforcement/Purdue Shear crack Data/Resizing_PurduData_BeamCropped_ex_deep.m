...
files=dir(fullfile('Sorted_Segmented_ex_deep', '*.png'));
...
Purdu_Scaled_segmented_cropped_line_ex_deep=cell(length(files),1);


for jj=1:length(files)
    
%     filename=files(jj).name;
    fname = strcat('./Sorted_Segmented_ex_deep/',char(FILES(jj)));
...
%     BW2=logical(BW2);
    ...
    Widden_fname = strcat('./Widden_Sorted_Segmented_BeamCropped_line_ex_deep/',char(FILES(jj)));
...
%         Island_Scaled_segmented_ex_postcap_cropped_line{jj,1}=I;
        Purdu_Scaled_segmented_cropped_line_ex_deep{jj,1}=I;

clear n m I
end
save Purdu_Scaled_segmented_cropped_line_ex_deep.mat;
toc